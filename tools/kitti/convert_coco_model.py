...
def clip_weights_from_pretrain_of_coco_to_kitti(f, out_file):
	""""""
	...
	# Kitti of fine categories for pretty print
	KITTI_FINE_CATEGORIES = [
	    "__background__",
	    "Car",
	    "Cyclist",
	    "Pedestrian",
	...
	kitti_cats = KITTI_FINE_CATEGORIES
	coco_cats_to_inds = dict(zip(coco_cats, range(len(coco_cats))))
	kitti_cats_to_inds = dict(
		zip(kitti_cats, range(len(kitti_cats)))
	)
	...
	cls_score = nn.Linear(representation_size, len(kitti_cats))
	...
	num_bbox_reg_classes = 2 if class_agnostic else len(kitti_cats)
	...
	mask_fcn_logits = nn.Conv2d(dim_reduced, len(kitti_cats), 1, 1, 0)
	...
	def _copy_weight(src_weight, dst_weight):
		for ix, cat in enumerate(kitti_cats):
...
if __name__=="__main__":
    # clip_weights_from_pretrain_of_coco_to_kitti(
    #     "checkpoints/pretrained/e2e_faster_rcnn_R_50_FPN_1x.pth",
    #     "checkpoints/pretrained/e2e_faster_rcnn_R_50_FPN_1x_kitti.pth"
    # )
    clip_weights_from_pretrain_of_coco_to_kitti(
        "checkpoints/pretrained/e2e_mask_rcnn_R_50_FPN_1x.pth",
        "checkpoints/pretrained/e2e_mask_rcnn_R_50_FPN_1x_kitti.pth"
    )