...
from ..backbone.monodepth2 import build_monodepth2
...
class GeneralizedRCNNLRMT(nn.Module):
    """
    ...
    def __init__(self, cfg):
        super(GeneralizedRCNNLRMT, self).__init__()
        self.cfg = cfg.clone()

        self.backbone = build_backbone(cfg)
        if not cfg.MODEL.DEPTHNET_ON:
            raise ValueError("GeneralizedRCNNLRMT Requires DEPTHNET_ON")

        if cfg.MODEL.DEPTHNET.MONODEPTH2_ON:
            self.depthnet = build_monodepth2(cfg)
            # self.depthnet_encoder = build_depthnet_decoder(cfg)
    ...
    def forward(self, images, targets=None, images_right=None, targets_right=None):
        """
        Arguments:
        ...
        if not self.training and self.cfg.TEST.TARGETS_AS_PROPOSALS:
