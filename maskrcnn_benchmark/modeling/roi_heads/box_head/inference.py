...
from maskrcnn_benchmark.structures.boxlist_ops import boxlist_nms, boxlist_nms_idx
...
class LRPostProcessor(nn.Module):
    """
    From a set of classification scores, box regression and proposals,
    computes the post-processed boxes, and applies NMS to obtain the
    final results
    ...
    def __init__(
        ...
        Arguments:
            ...
            box_coder (BoxCoder)
        """
        super(LRPostProcessor, self).__init__()
        self.score_thresh = score_thresh
        self.nms = nms
        self.detections_per_img = detections_per_img
        if box_coder is None:
    ...
    def forward(self, x, boxes):
        ...
        Returns:
            results (list[BoxList]): one BoxList for each image, containing
                the extra fields labels and scores
        """
        class_logits, box_regression_left, box_regression_right = x
        class_prob = F.softmax(class_logits, -1)
        ...
        if self.cls_agnostic_bbox_reg:
            box_regression_left = box_regression_left[:, -4:]
            box_regression_right = box_regression_right[:, -4:]
        ...
        proposals_right = self.box_coder.decode(
            box_regression_right.view(sum(boxes_per_image), -1), concat_boxes
        )
        if self.cls_agnostic_bbox_reg:
            proposals_left = proposals_left.repeat(1, class_prob.shape[1])
            proposals_right = proposals_right.repeat(1, class_prob.shape[1])
        ...
        for prob, boxes_left_per_img, boxes_right_per_img, image_shape in zip(
            class_prob, proposals_left, proposals_right, image_shapes
        ):
            boxlist_left = self.prepare_boxlist(boxes_left_per_img, prob, image_shape)
            boxlist_left = boxlist_left.clip_to_image(remove_empty=False)
            boxlist_right = self.prepare_boxlist(boxes_right_per_img, prob, image_shape)
            ...
            results_left.append(boxlist_left)
            results_right.append(boxlist_right)
    ...
    def filter_results(self, boxlist_left, boxlist_right, num_classes):
        ...
        boxes_union = boxlist_union(boxlist_left, boxlist_right).bbox.reshape(-1, num_classes * 4)
        ...
        for j in range(1, num_classes):
            ...
            # if j==1:print(inds_all[:, j])
            boxlist_for_class = BoxList(boxes_j, boxlist_left.size, mode="xyxy")
            boxlist_for_class_left = BoxList(boxes_j_left, boxlist_left.size, mode="xyxy")
            boxlist_for_class_right = BoxList(boxes_j_right, boxlist_left.size, mode="xyxy")
            boxlist_for_class.add_field("scores", scores_j)
            boxlist_for_class_left.add_field("scores", scores_j)
            boxlist_for_class_right.add_field("scores", scores_j)
            idxs = boxlist_nms_idx(
                boxlist_for_class, self.nms
            )
...
def make_roi_box_lr_post_processor(cfg):
    use_fpn = cfg.MODEL.ROI_HEADS.USE_FPN
