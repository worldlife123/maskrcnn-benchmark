...
class RPNPostProcessor(torch.nn.Module):
    ...
    def forward(self, anchors, objectness, box_regression, targets=None):
        ...
        # print([len(b[0]) for b in boxlists])
...
class RPNLRPostProcessor(torch.nn.Module):
    """
    Performs post-processing on the outputs of the RPN boxes, before feeding the
    proposals to the heads
    """
    ...
    def __init__(
        ...
        Arguments:
            ...
            fpn_post_nms_top_n (int)
        """
        super(RPNLRPostProcessor, self).__init__()
        self.pre_nms_top_n = pre_nms_top_n
        self.post_nms_top_n = post_nms_top_n
        self.nms_thresh = nms_thresh
        self.min_size = min_size
    ...
    def forward(self, anchors, objectness, box_regression_left, box_regression_right, targets_left=None, targets_right=None):
        """
        Arguments:
            anchors: list[list[BoxList]]
            objectness: list[tensor]
        ...
        Returns:
            boxlists (list[BoxList]): the post-processed anchors, after
                applying box decoding and NMS
        """
        sampled_boxes, sampled_boxes_right = [], []
        num_levels = len(objectness)
        anchors = list(zip(*anchors))
        for a, o, bl, br in zip(anchors, objectness, box_regression_left, box_regression_right):
            bsl, bsr = self.forward_for_single_feature_map(a, o, bl, br)
            sampled_boxes.append(bsl)
            sampled_boxes_right.append(bsr)

        boxlists_left = list(zip(*sampled_boxes))
        # print([len(b[0]) for b in boxlists_left])
        boxlists_left = [cat_boxlist(boxlist) for boxlist in boxlists_left]
        # print([len(boxlist) for boxlist in boxlists_left])

        if num_levels > 1:
            boxlists_left = self.select_over_all_levels(boxlists_left)

        # append ground-truth bboxes to proposals
        if targets_left is not None: # and self.training:
            boxlists_left = self.add_gt_proposals(boxlists_left, targets_left)

        boxlists_right = list(zip(*sampled_boxes_right))
        # print([len(b[0]) for b in boxlists_right])
        boxlists_right = [cat_boxlist(boxlist) for boxlist in boxlists_right]
        # print([len(boxlist) for boxlist in boxlists_right])

        if num_levels > 1:
            boxlists_right = self.select_over_all_levels(boxlists_right)

        # append ground-truth bboxes to proposals
        if targets_right is not None: # and self.training:
...
def make_rpn_lr_postprocessor(config, rpn_box_coder, is_train):
    fpn_post_nms_top_n = config.MODEL.RPN.FPN_POST_NMS_TOP_N_TRAIN
    if not is_train:
        fpn_post_nms_top_n = config.MODEL.RPN.FPN_POST_NMS_TOP_N_TEST
    ...
    nms_thresh = config.MODEL.RPN.NMS_THRESH
    min_size = config.MODEL.RPN.MIN_SIZE
    box_selector = RPNLRPostProcessor(
        pre_nms_top_n=pre_nms_top_n,
        post_nms_top_n=post_nms_top_n,
        nms_thresh=nms_thresh,
        min_size=min_size,
