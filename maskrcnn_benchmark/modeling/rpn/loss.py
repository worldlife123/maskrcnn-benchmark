...
class RPNLRLossComputation(object):
    """
    This class computes the RPN loss.
    """
    def __init__(self, proposal_matcher, fg_bg_sampler, box_coder,
    ...
    def match_targets_to_anchors(self, anchor, target_left, target_right, copied_fields=[]):
        # with torch.no_grad():
    ...
    def prepare_targets(self, anchors, targets_left, targets_right):
        labels = []
        regression_targets = []
        regression_targets_right = []
        for anchors_per_image, targets_left_per_image, targets_right_per_image in zip(anchors, targets_left, targets_right):

            matched_targets, matched_targets_right = self.match_targets_to_anchors(
                anchors_per_image, targets_left_per_image, targets_right_per_image, self.copied_fields
            )
            # [print(a.bbox[0]) for a in [targets_left_per_image, targets_right_per_image, anchors_per_image]]
            ...
            regression_targets_right_per_image = self.box_coder.encode(
                matched_targets_right.bbox, anchors_per_image.bbox
            )
            # [print(a.bbox[0]) for a in [matched_targets, matched_targets_right, anchors_per_image]]

            labels.append(labels_per_image)
            regression_targets.append(regression_targets_per_image)
            regression_targets_right.append(regression_targets_right_per_image)

        return labels, regression_targets, regression_targets_right


    def __call__(self, anchors, objectness, box_regression, box_regression_right, targets_left, targets_right):
        """
        Arguments:
            anchors (list[BoxList])
            objectness (list[Tensor])
        ...
        """
        anchors = [cat_boxlist(anchors_per_image) for anchors_per_image in anchors]
        labels, regression_targets, regression_targets_right = self.prepare_targets(anchors, targets_left, targets_right)
        sampled_pos_inds, sampled_neg_inds = self.fg_bg_sampler(labels)
        sampled_pos_inds = torch.nonzero(torch.cat(sampled_pos_inds, dim=0)).squeeze(1)
        sampled_neg_inds = torch.nonzero(torch.cat(sampled_neg_inds, dim=0)).squeeze(1)
...
class RPNHeadConsistencyLossComputation(object):
    """
    This class computes the RPN loss.
    """
    ...
    def __call__(self, anchors, objectness, box_regression, box_regression_right, objectness2, box_regression2, box_regression_right2):
        """
        Arguments:
            anchors (list[BoxList])
            objectness (list[Tensor])
...
def make_rpn_lr_loss_evaluator(cfg, box_coder):
    matcher = Matcher(
        cfg.MODEL.RPN.FG_IOU_THRESHOLD,
        cfg.MODEL.RPN.BG_IOU_THRESHOLD,
        allow_low_quality_matches=True,
    ...
    loss_evaluator = RPNLRLossComputation(
        matcher,
        fg_bg_sampler,
        box_coder,
        generate_rpn_labels
...
def make_rpn_lr_hc_loss_evaluator(cfg):
    return RPNHeadConsistencyLossComputation(cfg)