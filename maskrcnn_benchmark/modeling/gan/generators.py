...
from ..backbone import depthnet_decoder as depth_module
...
class SpatialTransformerGenerator(nn.Module):
    ...
    def __init__(self, cfg):
        """
        """
        super(SpatialTransformerGenerator, self).__init__()
        # TODO:
        # self.model = 
    ...
    def forward(self, input, invert_disp=False):
        disps = self.depth_decoder(input)["disp_output"]

        # transform
        transformed_input = []
        for image, disp in zip(input, disps):
            ...
            # print(x_base.shape, x_shifts.shape, y_base.shape)
            flow_field = torch.stack((x_base + x_shifts, y_base), dim=3)
            # In grid_sample coordinates are assumed to be between -1 and 1
            pred = F.grid_sample(image, 2*flow_field - 1, mode='bilinear',
                                padding_mode='zeros')
            transformed_input.append(pred)
        
        return tuple(transformed_input)

_DISCRIMINATOR_META_ARCHITECTURES = {
    "SpatialTransformerGenerator": SpatialTransformerGenerator,
}
